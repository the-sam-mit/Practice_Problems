...
int vis[SIZE],disc[SIZE],low[SIZE],ap_check[SIZE];
vector<int> v[SIZE];
int tim = 0;
...
void dfs(int x, int par){
	...
	for(auto it: v[x]){
		...
		if(vis[child] == 0){
			...
			if(low[child] >= disc[x] && par != -1) ap_check[x]=1;
			children++;
		}
		else low[x] = min(low[x], disc[child]);
	}
	if(par == -1 && children > 1) ap_check[x]=1;
}
...
int main(){
	...
	while(q--){
		int val;
		cin>>val;
		if(ap_check[val]==1) cout<<"Satisfied "<<sz(v[val])<<"\n";
