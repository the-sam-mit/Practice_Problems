...
ll power(ll a,ll b){if(b==0) return 1;  if(b==1) return a%M; ll x=power(a,b/2); if(b&1){return ((((a%M)*(x%M))%M)*(x%M))%M;}return ((x%M)*(x%M))%M;}
...
int main(){
	...
	rep(i,0,sz(s)-1){
		if(!isVowel(s[i])){
			cnt++;
			if(cnt>=3 && (!(s[i]==s[i-1] && s[i-1]==s[i-2]))) {ans+=' ';cnt=1;}
			else if((s[i]==s[i-1] && s[i-1]==s[i-2])) cnt=2;
		}
		else cnt=0;
		ans+=s[i];
	}
