...
int main(){
    ll n,m;
    cin>>n>>m;
    assert(n<=SIZE && m<=n && 1<=n && 1<=m);
    ll a[n+1];
    inp0(a,n)
    rep(i,0,n-1) assert((-INF <= a[i]) && (INF >= a[i]));
    ll b[n+1], c[n+1], sum[n+1];
    ll x = 0;
    mem(b,0);mem(c,0);mem(sum,0);
    rep(i,1,n-1){
    ...
    x = 0;
    nrep(i,0,n-2){
        c[i] = c[i+1] + (x*(a[i+1]-a[i]));
        if((n-1-(i+1))%m==0){
            x++;
            c[i] += a[i+1] - a[i];
        }
    }
    rep(i,0,n-1) sum[i] = 2*(b[i] + c[i]);
