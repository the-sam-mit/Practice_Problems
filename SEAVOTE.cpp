...
// ll powerM(ll a,ll b){if(b==0) return 1;  if(b==1) return a%M; ll x=power(a,b/2); if(b&1){return ((((a%M)*(x%M))%M)*(x%M))%M;}return ((x%M)*(x%M))%M;}
...
int main(){
	testcase(t){
		...
		ll sum=0;
		int num=0;
		rep(i,1,n){ sum+=b[i];if(b[i]!=0) num++;}
		if((sum >= 100) && (sum-num < 100)) cout<<"YES\n";
		else cout<<"NO\n";
	}
}
